...
<body>
	<% response.setHeader("cache-control","no-cache,no-store,must-revalidate"); //HTTP 1.1
        ...
        s=(Student)session.getAttribute("erStuData"); %>

	<%@include file="/header.jsp"%>
	<% if(session.getAttribute("idAvail")!=null){ out.print("<br>");
            out.print("<center><font size='4' color='red'>");
                out.print("Id is Already Available Try with Another Id --- <a href='findstudent?find="+s.getId()+"'>View Details of id["+s.getId()+"]</a>");
		...
		<div class="formbody">
			...
			<form action="registernewstudent" method="post">
				<label for="id">ID: </label><input type="text"
					value="<%=s.getId()%>" name="id" id="id" autofocus placeholder="Id" />
				<label for="name">Name: </label><input type="text" id="name"
					...
					placeholder="Address" /> <input type="submit" id="" value="create" />
			</form>
		</div>
	</div>
